...
void EnFz_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->unk_BD7 != 0) {
        ...
        gSPSegment(POLY_XLU_DISP++, 0x08,
                   Gfx_TwoTexScroll(play->state.gfxCtx, G_TX_RENDERTILE, 0, play->state.frames % 128, 32, 32, 1, 0,
                                    (play->state.frames * 2) % 128, 32, 32));
...
void func_80934464(EnFz* this, PlayState* play) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->unk_23C); i++, ptr++) {
        if (ptr->unk_00 > 0) {
            ...
            gSPSegment(POLY_XLU_DISP++, 0x08,
                       Gfx_TwoTexScroll(play->state.gfxCtx, G_TX_RENDERTILE, (ptr->unk_01 + (i * 3)) * 3,
                                        (ptr->unk_01 + (i * 3)) * 15, 32, 64, 1, 0, 0, 32, 32));
